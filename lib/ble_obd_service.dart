...
class BleObdService {
  ...
  Future<void> _requestBluetoothPermissions() async {
    if (Platform.isAndroid) {
      ...
      if (denied.isNotEmpty) {
        ...
        throw Exception('يجب السماح بصلاحيات Bluetooth للبحث عن أجهزة OBD.');
      }
      return;
    }
    ...
    if (Platform.isIOS) {
      ...
      if (denied.isNotEmpty) {
        throw Exception('يجب السماح بصلاحية Bluetooth للبحث عن أجهزة OBD.');
      }
    }
  }
  ...
  Future<void> scan({
    ...
    _scanSubscription = FlutterBluePlus.onScanResults.listen(
      (results) {
        for (final result in results) {
          if (result.rssi < minRssi) continue;
  ...
  Future<void> connect(BluetoothDevice device) async {
    // 1. إيقاف الفحص والانتظار قليلاً لمنع خطأ 133
    await stopScan();
    await disconnect();
    ...
    try {
      ...
      // تأخير بسيط بعد الاتصال للتركيز واستقرار طبقة GATT
      await Future.delayed(const Duration(milliseconds: 300));

      // اكتشاف جميع الخدمات تلقائياً بدون تقييد بـ ABF0
      final services = await device.discoverServices();
      ...
      if (writeChar == null || notifyChar == null) {
        throw const ObdProtocolException('لم يتم العثور على خصائص القراءة/الكتابة المناسبة للجهاز.');
      }

      _commandCharacteristic = writeChar;
      _responseCharacteristic = notifyChar;
      ...
      // تهيئة ELM327 العامة
      await sendCommand('ATZ\r', timeout: const Duration(seconds: 4));
  ...
  Future<String> sendCommand(
    ...
    try {
      final formattedCmd = command.endsWith('\r') ? command : '$command\r';
      final bytes = utf8.encode(formattedCmd);
  ...
  void _onBytesReceived(List<int> bytes) {
    ...
    _quietTimer?.cancel();
    _quietTimer = Timer(const Duration(milliseconds: 200), () {
      if (!completer.isCompleted && _responseBuffer.isNotEmpty) {
        completer.complete(_responseBuffer.toString());
      }
    });
